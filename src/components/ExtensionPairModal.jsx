...
import { SCOPE_LABEL_KEYS } from '../lib/extensions';
import './permissions-modal.less';

/**
 * ExtensionPairModal — an app on this computer asked to connect over the
 * local extension API. Shows its self-reported name and exactly what it
 ...
 */
export function ExtensionPairModal({ name, scopes, onAllow, onDeny }) {
    ...
    <div className="pm-backdrop" onClick={onDeny} role="presentation">
      ...
      >
        <div className="pm-head">
          ...
          <span className="pm-title" id="xp-title">
            {t('extensions.pairTitle', { name })}
          </span>
        </div>

        <p className="pm-intro">{t('extensions.pairIntro')}</p>

        <div className="pm-rows">
          {scopes.map((scope) => (
            <div className="pm-row" key={scope}>
              <div className="pm-row-info">
                <b>{t(`extensions.scopes.${SCOPE_LABEL_KEYS[scope]}`)}</b>
                <span>{t(`extensions.scopeHints.${SCOPE_LABEL_KEYS[scope]}`)}</span>
              </div>
            </div>
          ))}
        </div>
