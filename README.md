...
**Are you on one of the `⬜ Untested` rows — or a distro/version not listed?**
Please run the quick check and report back: open a
...
Wayland *and* X11). Confirmed reports get added here with credit.

> The table is generated from [`site/src/data/compat.js`](site/src/data/compat.js)
> via `npm run gen:compat` — edit that file (don't hand-edit the table) and the
> [marketing site](https://get.eyeread.in) updates from the same source.
